...
const int demo_protocols[] = {66, 67, PROTOCOL_VERSION, NEW_PROTOCOL_VERSION,
                              0};
...
qboolean gw_minimized =
    qfalse; // this will be always true for dedicated servers
#ifndef DEDICATED
...
void QDECL Com_Error(errorParm_t code, const char *fmt, ...) {
  ...
  if (code != ERR_DISCONNECT && code != ERR_NEED_CD) {
    // we can't recover from ERR_FATAL so there is no recipients for
    // com_errorMessage also if ERR_FATAL was called from S_Malloc - CopyString
    // for a long (2+ chars) text will trigger recursive error without proper
...
qboolean Com_EarlyParseCmdLine(char *commandLine, char *con_title,
  ...
  *con_title = '\0';
  ...
  for (i = 0; i < com_numConsoleLines; i++) {
    ...
    if (!Q_stricmpn(Cmd_Argv(0), "set", 3) &&
        !Q_stricmp(Cmd_Argv(1), "con_title")) {
      com_consoleLines[i][0] = '\0';
    ...
    if (!Q_stricmpn(Cmd_Argv(0), "set", 3) &&
        !Q_stricmp(Cmd_Argv(1), "rconPassword2")) {
      com_consoleLines[i][0] = '\0';
...
// Check for "safe" on the command line, which will skip loading of
// q3config.cfg.
...
// Adds command line parameters as script statements. Commands are separated by
// + signs. Returns qtrue if any late commands were added, which will keep the
...
#ifdef ZONE_DEBUG
  if (fb->next == NULL || fb->prev == NULL || fb->next == fb ||
      fb->prev == fb) {
    Com_Error(ERR_FATAL,
              "RemoveFree: bad pointers fb->next: %p, fb->prev: %p\n", fb->next,
              fb->prev);
  }
#endif
...
static void Z_ClearZone(memzone_t *zone, memzone_t *head, int size,
                        int segnum) {
  memblock_t *block;
  int min_fragment;
  ...
  zone->freelist_medium.next = zone->freelist_medium.prev =
      &zone->freelist_medium;
...
#ifdef ZONE_DEBUG
void *Z_TagMallocDebug(int size, memtag_t tag, char *label, char *file,
                       int line) {
  int allocSize;
#else
...
#define MEM_STATIC(chr)                                                        \
  {                                                                            \
    {NULL, NULL, PAD(sizeof(memstatic_t), 4), TAG_STATIC, ZONEID}, {           \
      chr, '\0'                                                                \
    }                                                                          \
  }
...
static const memstatic_t numberstring[] = {
    MEM_STATIC('0'), MEM_STATIC('1'), MEM_STATIC('2'), MEM_STATIC('3'),
    MEM_STATIC('4'), MEM_STATIC('5'), MEM_STATIC('6'), MEM_STATIC('7'),
    MEM_STATIC('8'), MEM_STATIC('9')};
#endif // USE_STATIC_TAGS

// NOTE: Never write over the memory CopyString returns because memory from a
// memstatic_t might be returned.
...
static const char *tagName[TAG_COUNT] = {
    "FREE",   "GENERAL",  "PACK",    "SEARCH-PATH", "SEARCH-PACK", "SEARCH-DIR",
...
static void Zone_Stats(const char *name, const memzone_t *z,
                       qboolean printDetails, zone_stats_t *stats) {
  const memblock_t *block;
  const memzone_t *zone;
  zone_stats_t st;
...
void Com_Meminfo_f(void) {
  ...
  Zone_Stats("main", mainzone,
             !Q_stricmp(Cmd_Argv(1), "main") || !Q_stricmp(Cmd_Argv(1), "all"),
             &st);
  Com_Printf("%8i bytes total main zone\n\n", mainzone->size);
  Com_Printf(
      "%8i bytes in %i main zone blocks%s\n", st.zoneBytes, st.zoneBlocks,
  ...
  Zone_Stats("small", smallzone,
             !Q_stricmp(Cmd_Argv(1), "small") || !Q_stricmp(Cmd_Argv(1), "all"),
             &st);
  Com_Printf("%8i bytes total small zone\n\n", smallzone->size);
  Com_Printf(
      "%8i bytes in %i small zone blocks%s\n", st.zoneBytes, st.zoneBlocks,
      st.zoneSegments > 1 ? va(" and %i segments", st.zoneSegments) : "");
  Com_Printf("        %8i bytes in %i free blocks\n", st.freeBytes,
             st.freeBlocks);
  if (st.freeBlocks > 1) {
...
static void Com_InitZoneMemory(void) {
  ...
  cv = Cvar_Get("com_zoneMegs", XSTRING(DEF_COMZONEMEGS),
...
static void Com_InitHunkMemory(void) {
  ...
  cv = Cvar_Get("com_hunkMegs", XSTRING(DEF_COMHUNKMEGS),
                CVAR_LATCH | CVAR_ARCHIVE);
...
// The temp space is no longer needed.  If we have left more touched but unused
// memory on this side, have future permanent allocs use this side.
...
void Com_Init(const char *commandLine) {
  ...
  Cvar_SetDescription(vm_rtChecks, "Runtime checks in compiled vm code, "
...
#ifndef DEDICATED
  com_maxfps =
      Cvar_Get("com_maxfps", "125", 0); // try to force that in some light way
  ...
  com_version =
      Cvar_Get("version", s, CVAR_PROTECTED | CVAR_ROM | CVAR_SERVERINFO);

  // this cvar is the single entry point of the entire extension system
  Cvar_Get("//trap_GetValue", va("%i", COM_TRAP_GETVALUE),
           CVAR_PROTECTED | CVAR_ROM | CVAR_NOTABCOMPLETE);

  Sys_Init();

  // CPU detection
  Cvar_Get("sys_cpustring", "detect",
           CVAR_PROTECTED | CVAR_ROM | CVAR_NORESTART);
...
extern int timeBeforeClient;
extern int timeAfter;
